...
/* =========================================
   ...
   ========================================= */
const categorias = [
    { titulo: "Electrónica y Gaming", productos: [{ id: 1, nombre: "Laptop Gamer 15.6'' 16GB RAM", precio: "$15,299.00" }, { id: 2, nombre: "Consola de Videojuegos 1TB Blanco", precio: "$8,499.00" }, { id: 3, nombre: "Audífonos Inalámbricos Over-Ear", precio: "$1,299.00" }, { id: 4, nombre: "Smart TV 55 Pulgadas 4K Ultra HD", precio: "$6,999.00" }, { id: 5, nombre: "Monitor Curvo 27'' 144Hz", precio: "$4,500.00" }] },
    { titulo: "Hogar y Electrodomésticos", productos: [{ id: 6, nombre: "Refrigerador 14 Pies Cúbicos", precio: "$8,990.00" }, { id: 7, nombre: "Horno de Microondas Acero Inox", precio: "$1,599.00" }, { id: 8, nombre: "Licuadora 10 Velocidades", precio: "$850.00" }, { id: 9, nombre: "Silla de Oficina Ergonómica", precio: "$1,850.00" }, { id: 10, nombre: "Colchón Matrimonial Memory Foam", precio: "$3,200.00" }] },
    { titulo: "Despensa Básica", productos: [{ id: 11, nombre: "Aceite Nutrioli 946 ml", precio: "$45.00" }, { id: 12, nombre: "Arroz Súper Extra 1 Kg", precio: "$22.50" }, { id: 13, nombre: "Leche Entera 1 Litro", precio: "$26.00" }, { id: 14, nombre: "Cereal Zucaritas 700g", precio: "$65.00" }, { id: 15, nombre: "Café Soluble Nescafé Clásico 225g", precio: "$110.00" }] }
];
...
function initApp() {
    ...
    $('detail-add-cart').onclick = function(e) {
        if(productoActual) addToCart(productoActual.id, productoActual.nombre, productoActual.precio, e);
    };
}
...
function abrirCategoria(nombreCat) {
    ...
    let gridHTML = '';
    for(let i=1; i<=30; i++) {
        gridHTML += generarTarjeta({ id: 100+i, nombre: `${nombreCat} Producto ${i}`, precio: `$${(Math.random()*500).toFixed(2)}` });
    }
...
function comprarAhoraProdActual() {
    if(productoActual) comprarRapido(productoActual.id, productoActual.nombre, productoActual.precio);
}

function toggleFav(id, nombre, precio, event) {
    const btn = event.currentTarget;
    const index = favoritos.findIndex(f => f.id === id);
    if(index === -1) { favoritos.push({ id, nombre, precio }); btn.style.backgroundColor = 'var(--danger)'; } 
    else { favoritos.splice(index, 1); btn.style.backgroundColor = 'var(--accent-yellow)'; }
}
...
function abrirCheckout() {
    let total = 0;
    let resumenHTML = '<ul style="list-style: none; padding: 0; margin: 0; font-size: 0.95rem;">';
...
function renderFavs() {
    ...
    container.innerHTML = favoritos.length === 0 ? '<p>No tienes artículos en favoritos.</p>' : favoritos.map(item => `<div class="list-item"><span>${item.nombre}</span><button class="btn-buy" style="padding: 0.5rem 1rem; border: 2px solid var(--text-blue); color: var(--text-blue); border-radius: 20px;" onclick="openProduct(${item.id}, '${item.nombre.replace(/'/g, "\\'")}', '${item.precio}')">Ver</button></div>`).join('');
}
...
function setupSidebar() {
    const toggleMenu = () => { $('sidebar').classList.toggle('active'); $('sidebarOverlay').classList.toggle('active'); };
